package top.kairuiyang.picture.service.impl;

import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import top.kairuiyang.commons.entity.File;
import top.kairuiyang.commons.entity.FileSort;
import top.kairuiyang.commons.entity.SystemConfig;
import top.kairuiyang.picture.global.MessageConf;
import top.kairuiyang.picture.global.SQLConf;
import top.kairuiyang.picture.global.SysConf;
import top.kairuiyang.picture.mapper.FileMapper;
import top.kairuiyang.picture.service.*;
import top.kairuiyang.picture.util.AboutFileUtil;
import top.kairuiyang.picture.util.FeignUtil;
import top.kairuiyang.utils.FileUtils;
import top.kairuiyang.utils.JsonUtils;
import top.kairuiyang.utils.ResultUtil;
import top.kairuiyang.utils.StringUtils;
import top.kairuiyang.base.enums.EFilePriority;
import top.kairuiyang.base.enums.EOpenStatus;
import top.kairuiyang.base.enums.EStatus;
import top.kairuiyang.base.exception.exceptionType.InsertException;
import top.kairuiyang.base.global.Constants;
import top.kairuiyang.base.global.ErrorCode;
import top.kairuiyang.base.holder.RequestHolder;
...
public class FileServiceImpl extends SuperServiceImpl<FileMapper, File> implements FileService {
    ...
    public String getPicture(String fileIds, String code) {
        ...
        if (StringUtils.isEmpty(fileIds)) {
            ...
            QueryWrapper<top.kairuiyang.commons.entity.File> queryWrapper = new QueryWrapper<>();
            queryWrapper.in(SQLConf.UID, changeStringToString);
            List<top.kairuiyang.commons.entity.File> fileList = fileService.list(queryWrapper);
            if (fileList.size() > 0) {
                for (top.kairuiyang.commons.entity.File file : fileList) {
                    if (file != null) {
    ...
    public String uploadPictureByUrl(FileVO fileVO) {
        ...
        List<top.kairuiyang.commons.entity.File> lists = new ArrayList<>();
        //文件上传
        if (urlList != null && urlList.size() > 0) {
            for (String itemUrl : urlList) {
                ...
                top.kairuiyang.commons.entity.File file = new top.kairuiyang.commons.entity.File();
                file.setCreateTime(new Date(System.currentTimeMillis()));
                file.setFileSortUid(fileSort.getUid());
